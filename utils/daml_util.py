...
def get_sample_mixup_randomint(domains):
    indeces = torch.randperm(domains)
    return indeces.long()
...
def get_ratio_mixup_Dirichletint(domains, mixup_dir_list):
    RG = np.random.default_rng()
    return torch.from_numpy(
        RG.dirichlet(mixup_dir_list, size=domains)
    ).float()  # N * 3
...
def weighted_manual_CE(predictions, labels, weight):
    loss = -torch.sum(labels * torch.log_softmax(predictions, dim=1), dim=1)
