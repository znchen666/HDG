...
def get_params(alg, args, inner=False, alias=True, isteacher=False):
    ...
    elif "SCIPD" == args.algorithm:
        params = [
            {"params": alg.featurizer.parameters(), "lr": args.lr_decay1 * initlr},
            # {"params": alg.linear_project.parameters(), "lr": args.lr_decay1 * initlr},
            # {"params": alg.classifier.parameters(), "lr": args.lr_decay1 * initlr},
            {"params": alg.classifier.parameters(), "lr": args.lr_cls * initlr},
            # {"params": alg.s3_classifier.parameters(), "lr": args.lr_decay2 * initlr},
        ]
...
def get_optimizer(alg, args, inner=False, alias=True, isteacher=False):
    ...
    if "SCIPD" == args.algorithm:
        optimizer = []
        for i in range(len(params)):
            optimizer.append(torch.optim.SGD(
                params,
                lr=args.lr,
...
def get_scheduler(optimizer, args):
    if not args.schuse:
        return None
    if args.schusech == "cos" or args.algorithm == "MIRO":
        if isinstance(optimizer, list):
            scheduler = []
            for optim in optimizer:
                scheduler.append(torch.optim.lr_scheduler.CosineAnnealingLR(
                    optim, args.max_epoch * args.steps_per_epoch
                ))
        else:
